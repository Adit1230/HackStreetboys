        ...
        if troop_codes[troop.name] not in opp_data[0]:
            for i in range(8):
                if opp_data[0][i] == '':
                    ...
                    break       
        if troop_codes[troop.name] not in opp_data[1]:
...
def logic(arena_data:dict):
    ...
    if(wanted_troops[0] in deployable_troops and wanted_troops[1] in deployable_troops and wanted_troops[2] in deployable_troops) or team_signal:
        if team_signal == '2' and arena_data['MyTower'].total_elixir > troop_elixirs[2]:
            deploy_list.list_.append((wanted_troops[2], (25, positions[2])))
            team_signal = ''
        elif team_signal == '1' and arena_data['MyTower'].total_elixir > troop_elixirs[1]:
