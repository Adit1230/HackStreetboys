...
team_name = "AggressiveHackstreet(shivamv2)"
...
def calc_troopvtroop_adv(our_name, opp_name, our_pos, opp_pos, troop_stats):
    ...
    our_attack_speed = 1/troop_stats[our_name]["attack_speed"]
    opp_attack_speed = 1/troop_stats[opp_name]["attack_speed"]
    ...
    if troop_stats[opp_name]["type"] in troop_stats[our_name]["targets"] and troop_stats[our_name]["type"] in troop_stats[opp_name]["targets"]:
        if range_type == "Bad":
            our_health = our_health - (time_to_encounter * troop_stats[opp_name]["damage"] * opp_attack_speed) #missing oppo num?
        elif range_type == "Long":
            opp_health = opp_health - (time_to_encounter * troop_stats[our_name]["damage"] * our_attack_speed)
        ...
        if our_health <= 0:
            ...
            num_killed = (opp_hits * opp_attack_speed) / (our_hits * our_attack_speed)
        ...
        if troop_stats[opp_name]["targets"] == ["Building"]:
            ...
            our_hits = opp_num * math.ceil(opp_health / (our_num * troop_stats[our_name]["damage"]))
            num_killed = our_hits * our_attack_speed * time
        else:
...
def calc_troop_scores(arena_data, troop_stats):
...
def logic(arena_data: dict):
    ...
    def can_launch_push():
        ...
        # Launch push if we have significant elixir advantage or enemy is low on elixir
        return (my_elixir >= 8 and enemy_elixir <= 3) or my_elixir >= 9

    def deploy_push_combination(tank, support1, support2=None):
        ...
        # Deploy tank at the back
        deploy_list.list_.append((tank, (attack_x, 50)))
        
        # Deploy first support troop slightly ahead
        deploy_list.list_.append((support1, (attack_x, 45)))
        
        # Deploy second support troop if available
        if support2:
            deploy_list.list_.append((support2, (attack_x, 40)))
    ...
    troop_scores = calc_troop_scores(arena_data, get_troop_stats())
    ...
    for enemy in enemy_troops:
        ...
        if enemy_distance_to_tower < 20*factor:  # Use tower's attack range as threshold
            ...
            deployable_troop_scores = [troop_scores[troop]["score"] for troop in deployable_troops]
    ...
    if not deploy_list.list_ and can_launch_push():
        ...
        for tank, support1, support2, min_elixir in PUSH_COMBINATIONS:
            if (tank in deployable_troops and support1 in deployable_troops and 
                ...
                deploy_push_combination(tank, support1, support2)
                break

    # Third priority: Deploy offensive pairs if no immediate threats
    if not deploy_list.list_ and my_elixir >= 6:
    ...
    if not deploy_list.list_ and not enemy_troops and my_elixir >= 6:  # Reduced from 10 to 6
        # Choose between left, center, or right lane attack
        lane = np.random.choice(["left", "center", "right"])
